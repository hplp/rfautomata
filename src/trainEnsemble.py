...
if __name__ == '__main__':
	...
	parser.add_option('-c', '--canned', type='string', dest='canned', help='A canned dataset included in SKLEARN (mnist)')
	...
	parser.add_option('-l', '--leaves', type='int', dest='leaves', help='Max number of leaves of the decision tree learners')
	parser.add_option('-n', '--tree_n', type='int', dest='tree_n', help='Number of decision trees')
	parser.add_option('-j', '--njobs', type='int', dest='njobs', help='Number of jobs to run in parallel for fit and predict')
	parser.add_option('-v', '--verbose', action='store_true', default=False, dest='verbose', help='Verbose')
	parser.add_option('-r', '--report', type='string', dest='report', default='report.txt', help='Name of the report file')
	...
	if options.trainfile is None:
		if options.canned not in datasets.keys():
			raise ValueError("No valid training data filename specified; provide -t or a valid canned dataset")
			exit(-1)
		else:
			canned = True
	...
	if options.depth is None:
		...
		if options.leaves is None:
			raise ValueError("No valid tree depth or leaves specified; provide -d <max depth> or -l <max leaves>")
			exit(-1)
		else:
	...
	if options.verbose:
		logging.info("Shape of the loaded training data. X:(%d,%d), y:(%d)" %(X.shape[0], X.shape[1], y.shape[0]))
	...
	if options.verbose:
		logging.info("Shape of the loaded testing data. X:(%d,%d), y:(%d)" %(X_test.shape[0], X_test.shape[1], y_test.shape[0]))
